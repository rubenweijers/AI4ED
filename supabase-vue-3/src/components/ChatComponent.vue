        ...
        async sendMessage() {
            ...
            while (attempts < maxRetries && !success) {
                try {
                    ...
                    // Save chat history
                    // Luke's notes I don't think this is right, but it seems like it's been working
                    // when we declared                 this.profileData = profileData;
                    // we haven't generate the first response yet
                    await supabase.from('chat_history_duplicate').insert({
                        ...
                        question_number: profileData.question_queue[profileData.current_question_index || 0],
                    });
